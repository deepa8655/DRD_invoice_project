...
# Database setup
# ----------------------------
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:Drd%40123@localhost/invoice_db'
...
def index():
    ...
    # Get search and date filters from query parameters
    search = request.args.get('search', '').strip()
    start_date = request.args.get('start_date')
    end_date = request.args.get('end_date')

    # Start query (join with customer for filtering by name)
...
def new_invoice():
    ...
    if request.method == 'POST':
        ...
        db.session.commit()
        print(f"✅ Invoice {inv.invoice_no} created successfully")
        return redirect(url_for('index'))

    return render_template('invoice_form.html', customers=customers, today=today)
